...
from lib import utils, dvgo, dcvgo_bce as dcvgo, dmpigo
...
def render_viewpoints(model, render_poses, HW, Ks, ndc, render_kwargs,
    ...
    if savedir is not None:
        res_metric_path = os.path.join(savedir, "eval_metric.json")
        res_dict = {}
    ...
    if len(psnrs):
        ...
        if eval_ssim: res_dict['ssim'] = np.mean(ssims)
        if eval_lpips_vgg: res_dict['lpips (vgg)'] = np.mean(lpips_vgg)
        if eval_lpips_alex: res_dict['lpips (alex)'] = np.mean(lpips_alex)
        with open(res_metric_path, "w") as fp:
            json.dump(res_dict, fp)
            fp.close()
        print(f'saving eval results into {res_metric_path}')
...
def scene_rep_reconstruction(args, cfg, cfg_model, cfg_train, xyz_min, xyz_max, data_dict, stage, coarse_ckpt_path=None):
    ...
    rgb_tr, rays_o_tr, rays_d_tr, viewdirs_tr = rgb_tr.cuda(), rays_o_tr.cuda(), rays_d_tr.cuda(), viewdirs_tr.cuda()
    ...
    for global_step in trange(1+start, 1+cfg_train.N_iters):
        ...
        loss = 0.0
        mse_loss = cfg_train.weight_main * F.mse_loss(render_result['rgb_marched'], target)
        loss += mse_loss
        if cfg_train.bce_weight > 0:
            bce_loss = cfg_train.bce_weight * ce_func(render_result['rgb_marched'].clamp(0., 1-1e-3), target)
            loss += bce_loss
...
def train(args, cfg, data_dict):
    ...
    time_path = os.path.join(cfg.basedir, cfg.expname, 'time_record.json')
    time_dict = {}
    time_dict['time_recode'] = eps_time
    with open(time_path, "w") as fp:
        json.dump(time_dict, fp)
        fp.close()
    print(f'saving time usage into {time_path}')
