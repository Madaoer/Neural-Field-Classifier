...
class DirectContractedVoxGO(nn.Module):
    def __init__(self, xyz_min, xyz_max,
        ...
        rgb_seq = torch.arange(256).reshape(1, 1, 256)
        self.rgb_seq = rgb_seq.expand(9000000, 3, 256)
        ...
        # self.class_weight = torch.tensor([1/2**0, 1/2**1, 1/2**2, 1/2**3, 1/2**4, 1/2**5, 1/2**6, 1/2**7]).cuda()
        ...
        if rgbnet_dim <= 0:
            ...
            self.rgbnet = nn.Sequential(
                ...
                *[
                ...
                nn.Linear(rgbnet_width, channel),
            )
    ...
    def forward(self, rays_o, rays_d, viewdirs, global_step=None, is_train=False, pred_type='soft_pred', aug_type=None, **render_kwargs):
        '''Volume rendering
        ...
        if self.rgbnet is None:
            ...
            rgb_logit = rgb_logit.reshape(-1, 3, 256)
            rgb_prob = F.softmax(rgb_logit, dim=-1)
            # singlergb = torch.sum(rgb.reshape(-1, 3, rgb_channel) * self.class_weight  / self.class_weight.sum(), dim=-1)
            tmp_rgb_seq = self.rgb_seq[:rgb_prob.shape[0]]
            rgb_prob = rgb_prob * tmp_rgb_seq
            singlergb = torch.sum(rgb_prob, dim=-1) / rgb_channel
            rgb_prob = rgb_prob.reshape(-1, channel)
        # Ray marching
        rgb_marched = segment_coo(
                src=(weights.unsqueeze(-1) * rgb_prob),
                index=ray_id,
                out=torch.zeros([N, channel]),
                reduce='sum')
        tmp_rgb_seq = self.rgb_seq[:rgb_marched.shape[0]]
        rgb_marched = rgb_marched.reshape(-1, 3, 256)
        res_rgb = rgb_marched * tmp_rgb_seq
        res_rgb = torch.sum(res_rgb, dim=-1) / rgb_channel
        # res_rgb = torch.sum(rgb_marched.reshape(-1, 3, rgb_channel) * self.class_weight / self.class_weight.sum(), dim=-1)
        if render_kwargs.get('rand_bkgd', False) and is_train:
            res_rgb += (alphainv_last.unsqueeze(-1) * torch.rand_like(res_rgb))
        else:
            res_rgb += (alphainv_last.unsqueeze(-1) * render_kwargs['bg'])
