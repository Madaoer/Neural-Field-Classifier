...
class SurfaceModelConfig(ModelConfig):
    ...
    bce_loss_mult: float = 1.0
    reg_bce_loss_mult: float = 10.0
...
class SurfaceModel(Model):
    ...
    def get_loss_dict(self, outputs, batch, metrics_dict=None) -> Dict:
        ...
        if self.training:
            ...
            if self.config.bce_loss_mult > 0:
                bce_eps = 1e-3
                loss_dict["bce_loss"] = self.config.bce_loss_mult * self.bce_loss(outputs["rgb"].clamp(0., 1.0 - bce_eps), image.detach()).mean()
            # foreground mask loss
            if "fg_mask" in batch and self.config.fg_mask_loss_mult > 0.0:
                fg_label = batch["fg_mask"].float().to(self.device)
                weights_sum = outputs["weights"].sum(dim=1).clip(bce_eps, 1.0 - bce_eps)
